...
"${TOOLCHAIN}/clang" -arch arm64 -Os -isysroot "${SDK_PATH}" \
	-miphoneos-version-min=9.0 -c "${ROOT_DIR}/third_party/tdlib/build/weak_import_shims.c" -o "${COMPAT_O}"
echo "[+] ios10-weak-import compat shims: ${COMPAT_O}"
...
cmake "${TDLIB_SRC}" \
	...
	-DATOMICS_FOUND=TRUE \
	-DATOMICS_LIBRARIES="" \
	-DCMAKE_C_FLAGS="-Wno-error -Os -g -I${ROOT_DIR}/third_party/tdlib" \
	-DCMAKE_CXX_FLAGS="-Wno-error -Os -g -I${ROOT_DIR}/third_party/tdlib ${CXX_STDLIB_INC}" \
