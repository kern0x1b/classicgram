...
- (NSString *)format:(NSString *)str intlPrefix:(NSString *)intlPrefix trunkPrefix:(NSString *)trunkPrefix {
	...
	for (NSInteger i = 0; i < (int)[self.format length]; i++) {
...
+ (NSString *)strip:(NSString *)str {
	...
	for (NSInteger i = (int)[res length] - 1; i >= 0; i--) {
...
+ (TGPhoneFormat *)instance {
	static TGPhoneFormat *instance = nil;
...
- (id)initWithDefaultCountry:(NSString *)countryCode {
	if ((self = [super init])) {
		NSString *path = [[NSBundle mainBundle] pathForResource:@"PhoneFormats" ofType:@"dat"];
		_data = path.length ? [NSData dataWithContentsOfFile:path] : nil;
		if (_data.length < 16)
			return nil;
...
- (NSString *)format:(NSString *)orig implicitPlus:(bool)implicitPlus {
	NSString *str = [TGPhoneFormat strip:orig];
...
- (CallingCodeInfo *)callingCodeInfo:(NSString *)callingCode {
	...
	if (res == nil) {
		...
		if (num) {
			...
			for (NSInteger s = 0; s < setCnt; s++) {
				RuleSet *ruleSet = [[RuleSet alloc] init];
				NSInteger matchCnt = [self value16:offset];
				ruleSet.matchLen = matchCnt;
				offset += 2;
				NSInteger ruleCnt = [self value16:offset];
				offset += 2;
				NSMutableArray *rules = [NSMutableArray arrayWithCapacity:ruleCnt];
				for (NSInteger r = 0; r < ruleCnt; r++) {
...
- (void)parseDataHeader {
	NSInteger count = [self value32:0];
